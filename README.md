...
method = "basic"
username = "admin"
# Recommended: Argon2id hash generated by `ttyd-rs --hash-password`
...
- **Store the password as an Argon2id hash** — run `printf 'secret\n' | ttyd-rs --hash-password` and use the output as the `password` value; plaintext values still work but trigger a startup warning
